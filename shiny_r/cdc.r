...
ui <- fluidPage(
  titlePanel("Diabetes Diagnosis Age-adjusted Prevalence in CA by County"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$barPlot <- renderPlot({
    ...
    ggplot() +
      ...
      geom_hline(aes(yintercept = avg_value), linetype = "dashed", color = "tomato") +
      labs(title = 'Diagnosed Diabetes Age-adjusted Prevalence',
